...
import type { GroupByKey, Granularity } from "@/features/upcoming/hooks/useUpcomingFilters";
...
const weekDiff = (a: Date, b: Date) =>
  Math.round((startOfWeek(a).getTime() - startOfWeek(b).getTime()) / (7 * 86400000));
...
function timeBucket(
  ...
  const raw = field === "timePlanned" ? task.TimePlannedStart : task.TimeCompleted;
  if (!raw) {
    const label = field === "timeCompleted" ? "Not completed" : "No date";
    return { key: "__none", label, icon: "circle-dashed", tone: "muted", order: Number.MAX_SAFE_INTEGER };
  }
  ...
  if (isPlanned && !task.TimeCompleted) {
    ...
    if (overdue)
      return { key: "__overdue", label: "Overdue", icon: "circle-alert", tone: "danger", order: -1e9 };
  }
  ...
  if (gran === "day") {
    ...
    const ds = dayOnly(d);
    const label =
      diff === 0 ? "Today" : diff === 1 ? "Tomorrow" : diff === -1 ? "Yesterday" : fmtDayFull(ds);
    return {
      key: "d" + ds.getTime(),
      label,
      sublabel: diff === 0 || diff === 1 || diff === -1 ? fmtDay(ds) : undefined,
      icon: "calendar",
      order: diff,
    };
  }
  if (gran === "month") {
    const mDiff =
      ...
      (today.getFullYear() * 12 + today.getMonth());
    const label =
      mDiff === 0 ? "This month" : mDiff === 1 ? "Next month" : mDiff === -1 ? "Last month" : fmtMonth(d);
    return {
      key: "m" + d.getFullYear() + "-" + d.getMonth(),
      label,
      sublabel: mDiff === 0 || mDiff === 1 || mDiff === -1 ? fmtMonth(d) : undefined,
      icon: "calendar-days",
      order: mDiff,
    };
  }
  ...
  const label =
    diff === 0 ? "This week" : diff === 1 ? "Next week" : diff === -1 ? "Last week" : fmtWeekRange(ws);
  return {
    key: "w" + ws.getTime(),
    label,
    sublabel: diff === 0 || diff === 1 || diff === -1 ? fmtWeekRange(ws) : undefined,
    icon: "calendar",
    order: diff,
  };
}
...
function categoryBucket(
  ...
  switch (dim) {
    case "project": {
      const name = data.projectById[task.ProjectID]?.Name ?? `Project ${task.ProjectID}`;
      return { key: "p" + task.ProjectID, label: name, icon: "folder", order: task.ProjectID };
    }
    case "stage": {
    ...
    case "priority":
      return { key: "pri-" + task.Priority, label: task.Priority, priority: task.Priority, order: priRank(task.Priority) };
    case "type":
      return { key: "ty-" + task.Type.ID, label: task.Type.Name || "Unknown type", icon: task.Type.Icon, color: task.Type.Color, order: task.Type.ID };
    case "assignee":
      return task.Assignee
        ? { key: "a-" + task.Assignee, label: task.Assignee, icon: "user", order: task.Assignee.charCodeAt(0) }
        : { key: "a-none", label: "Not assigned", icon: "user", tone: "muted", order: 1e6 };
    case "checklist": {
      const projName = data.projectById[task.ProjectID]?.Name ?? `Project ${task.ProjectID}`;
      return {
...
function sortTasks(tasks: TaskWithProject[]): TaskWithProject[] {
  return [...tasks].sort((a, b) => {
    const ad = a.TimePlannedStart ? new Date(a.TimePlannedStart).getTime() : Infinity;
    const bd = b.TimePlannedStart ? new Date(b.TimePlannedStart).getTime() : Infinity;
...
export function buildGroups(
  ...
  for (const t of tasks) {
    const b = isTimeGroup
      ? timeBucket(t, groupBy as "timePlanned" | "timeCompleted", granularity, today)
  ...
  if (showEmpty && groupBy === "timePlanned" && granularity === "week") {
    ...
    const canon: Omit<TaskGroup, "tasks">[] = [
      { key: "__overdue", label: "Overdue", icon: "circle-alert", tone: "danger", order: -1e9 },
      { key: "w" + thisWeek.getTime(), label: "This week", sublabel: fmtWeekRange(thisWeek), icon: "calendar", order: 0 },
      { key: "w" + addDays(thisWeek, 7).getTime(), label: "Next week", sublabel: fmtWeekRange(addDays(thisWeek, 7)), icon: "calendar", order: 1 },
    ];
